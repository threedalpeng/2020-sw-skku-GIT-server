    ...
    <div class= "title">
      ...
      <v-row>
        <!-- <v-btn
          ...
          :loading="isloading"
          color="primary" dark>admin's_CCTV_01_
      ...
      <div class= "video">
        ...
        <img v-if="isloading" src= "../assets/UI_01/loading_img.png" align="left">
      </div>
      <figures v-bind:figures="{n_person, n_mask_off, n_mask_on,
                                n_mask_unknown, risk, congestion, n_cluster,
                                place_num}"/>
      <v-btn
        absolute
        right
        v-on:click="link_to_graph"
        id="b2" color="primary" dark>Analysis
        <v-icon color="white" dark right>{{ icons.mdiArrowRightBoldOutline }}</v-icon>
        </v-btn>
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    change_mode(){
      ...
      this.isloading= true;
      this.socket.emit('switch_room', this.previous_camera_id, this.destination_camera_id);
      console.log(this.previous_camera_id,this.destination_camera_id);
    },
    handler(){ //when browser or tab is killed
      axios.get('http://115.145.212.100:53344/api/realtime/kill');
    },
  },
  watch:{
    ...
    alert: function(val){
      if(alert == 1){
        ...
        // mask_audio.play(); backend not finished
      }
    },
    risk: function(val){
      if(val > 80 && this.isred == false){ // when risk val is over 80 UI background color blinks!
        this.red_sign();
      }
    },
    frame: function(val){ //if no recieved frame
      if(val != ''){
        this.getframe= true;
