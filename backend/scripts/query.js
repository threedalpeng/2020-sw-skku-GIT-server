...
async function testFunc() {
    ...
    await models.camera.create({
        ...
        mode: "live",
        access_path: "rtsp://skkutapo:skkuproject@192.168.0.9:554/stream1",
        room_size: 66,
        distance_criteria: 2,
        proper_n_people: 10,
...
async function checkAllAlerts() {
    const now = moment();
    let curSearchTime = moment("2020-09-21 00:05");
    let camera_id = 1;
    let alert_crit = await models.camera.findByPk(camera_id, {
        attributes: ['alarm_criteria']
    }).alarm_criteria;
    while (curSearchTime < now) {
        let end_time = moment(curSearchTime).subtract(1, 'minutes');
        let beg_time = moment(end_time).subtract(4, 'minutes');
        let data_last_five_minutes = await models.time_data.findAll({
            raw: true,
            logging: false,
            where: {
                camera_id: camera_id,
                ...
                captured_time: {
                    [Op.gte]: beg_time.format("HH:mm:00"),
                    [Op.lte]: end_time.format("HH:mm:00")
                }
            },
        ...
        if ((0.6 * total_risk + 0.4 * total_congestion) / cnt_data >= alert_crit) {
            /*
            await models.time_data.findOrCreate({
                where: {
                    camera_id: camera_id,
    ...
    for (let i = 0; i < 28; i++) {
        ...
        if (hr_cnt_data != 0) {
            ...
            await models.hourly_data.findOrCreate({
                where: {
                    camera_id: cam_id,
                    analyzed_time: cur_prc_hour.format("YYYY-MM-DD HH:mm:00")
                },
                defaults: {
                    avg_risk: hr_avg_risk,
                    max_risk: hr_max_risk,
        ...
        if (dy_cnt_data != 0) {
            ...
            await models.daily_data.findOrCreate({
                ...
                defaults: {
                    ...
                    safety_score: (10 - (0.04 * dy_avg_congestion + 0.06 * dy_avg_risk))
                }
            });
